...
function addMessage(username, badges, message, isAction, uid, msgId) {
    ...
    <div data-sender="${uid}" data-msgid="${msgId}" class="message-row {animationIn} animated" id="msg-${totalMessages}">
        <div class="user-box ${actionClass}">
			<div class="simbols-box">
                <div class="item x"><p>x</p></div>
                <div class="item blue">
			...
			<div class="name-box">${badges}${username}</div>
		</div>
        <div class="user-message ${actionClass}"><div class="message">${message}</div></div>
    </div>`);
    if (addition === "append") {
        if (hideAfter !== 999) {
  	...
  	console.log(window.document.querySelector('div.main-container'))
